"use client";

import React, { useEffect, useRef } from "react";
import { ArrowUpRight } from "lucide-react";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";

gsap.registerPlugin(ScrollTrigger);

interface CompactGoogleBannerProps {
  className?: string;
}

export function CompactGoogleBanner({ className = "" }: CompactGoogleBannerProps) {
  const bannerRef = useRef<HTMLDivElement>(null);
  const contentRef = useRef<HTMLDivElement>(null);
  const buttonRef = useRef<HTMLButtonElement>(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      // Animate banner entrance
      gsap.from(bannerRef.current, {
        y: 20,
        opacity: 0,
        duration: 0.8,
        ease: "power3.out",
        scrollTrigger: {
          trigger: bannerRef.current,
          start: "top 85%",
          once: true
        }
      });

      // Animate content
      gsap.from(contentRef.current, {
        x: -15,
        opacity: 0,
        duration: 0.6,
        delay: 0.2,
        ease: "power2.out",
        scrollTrigger: {
          trigger: contentRef.current,
          start: "top 85%",
          once: true
        }
      });

      // Animate button
      gsap.from(buttonRef.current, {
        scale: 0.9,
        opacity: 0,
        duration: 0.5,
        delay: 0.3,
        ease: "power2.out",
        scrollTrigger: {
          trigger: buttonRef.current,
          start: "top 85%",
          once: true
        }
      });

      // Button hover effect
      if (buttonRef.current) {
        buttonRef.current.addEventListener('mouseenter', () => {
          gsap.to(buttonRef.current, {
            scale: 1.05,
            duration: 0.2,
            ease: "power2.out"
          });
        });

        buttonRef.current.addEventListener('mouseleave', () => {
          gsap.to(buttonRef.current, {
            scale: 1,
            duration: 0.2,
            ease: "power2.out"
          });
        });
      }
    }, bannerRef);

    return () => ctx.revert();
  }, []);

  return (
    <div ref={bannerRef} className={`relative w-full overflow-hidden ${className}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 md:px-8 py-4 sm:py-6">
        <div className="relative overflow-hidden rounded-2xl shadow-2xl border border-blue-400/20">
          {/* Modern gradient background */}
          <div className="absolute inset-0 bg-gradient-to-r from-blue-600 via-blue-500 to-indigo-600" />
          <div className="absolute inset-0 bg-gradient-to-br from-blue-700/80 via-blue-600/70 to-indigo-700/80" />
          
          {/* Animated background elements */}
          <div className="absolute inset-0 overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-white/5 rounded-full blur-2xl animate-pulse" />
            <div className="absolute bottom-0 left-0 w-20 h-20 bg-white/3 rounded-full blur-xl animate-pulse" style={{ animationDelay: "1s" }} />
            <div className="absolute top-1/2 left-1/3 w-12 h-12 bg-white/4 rounded-full blur-lg animate-pulse" style={{ animationDelay: "0.5s" }} />
            
            {/* Grid pattern overlay */}
            <div className="absolute inset-0 opacity-20">
              <div className="h-full w-full" style={{
                backgroundImage: 'linear-gradient(rgba(255,255,255,0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.1) 1px, transparent 1px)',
                backgroundSize: '30px 30px'
              }} />
            </div>
          </div>

          {/* Content */}
          <div className="relative z-10 px-6 sm:px-8 py-4 sm:py-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              {/* Left Side - Content */}
              <div ref={contentRef} className="flex items-center gap-4">
                {/* Google Logo with glow effect */}
                <div className="relative flex-shrink-0">
                  <div className="absolute inset-0 bg-white/20 rounded-full blur-md" />
                  <div className="relative bg-white/10 backdrop-blur-sm rounded-full p-2 border border-white/20">
                    <svg width="32" height="32" viewBox="0 0 24 24" className="w-6 h-6 sm:w-8 sm:h-8">
                      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                    </svg>
                  </div>
                </div>

                {/* Text Content */}
                <div>
                  <h3 className="text-lg sm:text-xl font-bold text-white mb-1 leading-tight">
                    Add news-demand-tech.com as a trusted source for Google
                  </h3>
                  <div className="flex items-center gap-2">
                    <span className="text-xs sm:text-sm font-semibold text-white/90 uppercase tracking-wider">
                      GOOGLE INTEGRATION
                    </span>
                    <span className="text-xs text-white/60">•</span>
                    <span className="text-xs sm:text-sm text-white/80">
                      Preferred Source for US Market
                    </span>
                  </div>
                </div>
              </div>

              {/* Right Side - Button */}
              <div className="flex-shrink-0">
                <button 
                  ref={buttonRef}
                  className="inline-flex items-center gap-2 bg-white text-blue-600 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-semibold text-sm sm:text-base transition-all duration-300 shadow-xl hover:shadow-2xl hover:scale-105 group border border-white/20"
                >
                  <span>Add to Google</span>
                  <ArrowUpRight 
                    size={16} 
                    className="transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1" 
                  />
                </button>
              </div>
            </div>
          </div>

          {/* Decorative elements */}
          <div className="absolute top-2 right-2 w-2 h-2 bg-white/40 rounded-full animate-pulse" />
          <div className="absolute bottom-2 left-2 w-2 h-2 bg-white/30 rounded-full animate-pulse" style={{ animationDelay: "0.5s" }} />
          
          {/* Bottom gradient overlay */}
          <div className="absolute bottom-0 left-0 right-0 h-6 bg-gradient-to-t from-black/20 to-transparent" />
        </div>
      </div>
    </div>
  );
}
